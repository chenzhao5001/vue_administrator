<template>
  <el-container class="video">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <p>
                  <span>昵称:</span>
                  <span>{{scope.row.userInfoTemp.user_name}}</span>
                </p>
                <p>
                  <span>身份:</span>
                  <span>{{scope.row.userInfoTemp.user_type}}</span>
                </p>
                <p>
                  <span>学科:</span>
                  <span>{{scope.row.userInfoTemp.user_subject}}</span>
                </p>
                <p>
                  <span>阶段:</span>
                  <span>{{scope.row.userInfoTemp.user_grade_level}}</span>
                </p>
                <p>
                  <span>级别:</span>
                  <span>{{scope.row.userInfoTemp.user_level}}</span>
                </p>
                <p>
                  <span>UID:</span>
                  <span>{{scope.row.userInfoTemp.user_uid}}</span>
                </p>
              </template>
            </el-table-column>
            <el-table-column
              prop="articleInfoTemp"
              label="文章信息"
              min-width="240"
              align="center"
              class-name="column2"
            >
              <template slot-scope="scope">
                <p>
                  <span>标题:</span>
                  <span>{{scope.row.articleInfoTemp.title}}</span>
                </p>
                <p>
                  <span>学科:</span>
                  <span>{{scope.row.articleInfoTemp.subject}}</span>
                </p>
                <p>
                  <span>阶段:</span>
                  <span>{{scope.row.articleInfoTemp.grade_class}}</span>
                </p>
                <p>
                  <span>长度:</span>
                  <span>{{scope.row.articleInfoTemp.length}}</span>
                </p>
                <p>
                  <span>XY:</span>
                  <span>{{scope.row.articleInfoTemp.xy}}</span>
                </p>
                <p>
                  <span>AID:</span>
                  <span>{{scope.row.articleInfoTemp.aid}}</span>
                </p>
              </template>
            </el-table-column>
            <el-table-column
              prop="articlePathTemp"
              label="文章阅读"
              width="220"
              align="center"
              class-name="column3"
            >
              <template slot-scope="scope">
                <div class="imgbox">
                  <img
                    class="see"
                    src="../../../../assets/img/admin/commmon/eye.png"
                    @click="openExplor(scope.row.articlePathTemp.content,scope.$index)"
                  >
                  <img
                    class="pre"
                    :src="scope.row.articlePathTemp.title_pic1||scope.row.articlePathTemp.title_pic2||scope.row.articlePathTemp.title_pic3"
                    @click="openExplor(scope.row.articlePathTemp.content,scope.$index)"
                  >
                </div>
              </template>
    ...
    <div class="videobox" v-if="playing">
      <iframe scrolling="auto " :src="articleUrl" frameborder="0"></iframe>
    </div>
...
export default {
  ...
  computed: {
    getHeight() {
      return document.body.clientHeight - 88;
    }
  },
  watch: {
    result: {
  ...
  methods: {
    ...
    getErrorList() {
      this.$http({
        method: "get",
        url: "/admin/article_fail_reason_list"
      })
        .then(({ res, ok }) => {
          if (ok) {
            this.errorList = res;
        ...
        .catch(error => {
          alert("文章审核错误列表获取失败，请联系管理员");
        });
    },
    getList() {
      this.$http({
        url: "/admin/current_article_list"
      })
        .then(({ res, ok }) => {
          if (ok) {
            if (res.length == 0) {
